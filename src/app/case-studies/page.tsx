...
function ResultsBox({ results, accentColor }: { results: Array<{ value: string; label: string }>; accentColor: string }) {
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-5 text-[#10B981] font-semibold text-xs uppercase tracking-wider">
      ...
      <div className="grid grid-cols-3 gap-4">
        {results.map((result, idx) => (
          <div key={idx} className="text-center">
            <div className="font-headline text-3xl md:text-4xl font-bold leading-none mb-1.5" style={{ color: accentColor }}>
              {result.value}
            </div>
    ...
    <>
      ...
      <main id="main-content" role="main">
        ...
        >
          ...
          <div className="max-w-[1240px] mx-auto px-5 sm:px-6 md:px-8 lg:px-12 text-center relative z-10">
            <div className="text-cyan-400 text-xs font-mono font-semibold uppercase tracking-widest mb-3">Technology Stack</div>
            <h2 id="tech-stack-heading" className="font-headline text-4xl font-bold text-white mb-16">Tools We Build With</h2>

            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 max-w-4xl mx-auto">
              {techCategories.map((cat) => (
                <div key={cat.label}>
                  <div className="text-xs text-slate-500 uppercase tracking-widest mb-4 font-mono">{cat.label}</div>
                  <div className="flex flex-col gap-3">
                    {cat.items.map((item) => (
                      <div
                        key={item.name}
                        className="flex items-center gap-3 p-3 rounded-xl bg-white/[0.04] border border-white/[0.06] hover:bg-white/[0.08] hover:border-white/[0.12] transition-all group"
                        title={item.name}
                      >
                        <div
                        ...
                        >
                          <svg viewBox="0 0 24 24" fill={item.color} className="w-5 h-5">
                            <path d={item.path} />
                          </svg>
                        </div>
                        <span className="text-sm text-slate-300 font-medium group-hover:text-white transition-colors">{item.name}</span>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        >
          {/* Gradient orbs */}
          <div className="absolute top-0 left-1/4 w-96 h-96 bg-gradient-to-br from-cyan-500/20 to-transparent rounded-full blur-3xl" aria-hidden="true" />
          <div className="absolute bottom-0 right-1/4 w-72 h-72 bg-gradient-to-tl from-blue-400/20 to-transparent rounded-full blur-3xl" aria-hidden="true" />
